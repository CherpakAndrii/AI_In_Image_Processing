...
def face_encodings(image):
    return [numpy.array(face_encoder.compute_face_descriptor(image, face_landmark)) for face_landmark in
            face_landmarks(image)]


def nb_of_matches(known_encodings, unknown_encoding):
    distances = numpy.linalg.norm(known_encodings - unknown_encoding, axis=1)
